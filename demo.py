# a python 3 version of https://davywybiral.blogspot.com/2010/09/procedural-music-with-pyaudio-and-numpy.html
...
class Scale:
  ...
  def get(self, index):
    ...
    for i in range(index):
      note = note.transpose(intervals[i % len(self.intervals)])
    return note
  ...
  def index(self, note):
    ...
    while x.octave != note.octave or x.note != note.note:
      x = x.transpose(self.intervals[index % len(self.intervals)])
      index += 1
    return index
